...
describe('page titles', () => {
  it('uses the product name and tagline on home', () => {
    ...
    expect(documentTitle('/', t)).toBe('CivicHub — Building informed citizens');
  });
  ...
  it('prefers the more specific manage route over the public list', () => {
    ...
    expect(documentTitle('/articles/manage', t)).toBe('Manage articles · CivicHub');
  });

  it('titles auth screens', () => {
    expect(titleKeyForPath('/login')).toBe('nav.login');
    expect(documentTitle('/login', t)).toBe('Log in · CivicHub');
  });
});
