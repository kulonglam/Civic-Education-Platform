...
import type { CivicNews } from '../types/api';
...
export function NewsManagePage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<CivicNews | null>(null);
  const [deleteError, setDeleteError] = useState('');
  const [deleting, setDeleting] = useState(false);

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: queryKeys.newsManage,
    queryFn: async () => {
      const { data: res } = await newsService.list({ page_size: '100' });
  ...
  const confirmDelete = async () => {
    ...
    setDeleteError('');
    try {
      await newsService.remove(pendingDelete.id);
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        <EmptyState title={t('news.emptyManage')} />
      ) : (
        <div className="overflow-x-auto rounded-2xl border border-ink-100 bg-white dark:border-slate-800 dark:bg-slate-900">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-ink-50 text-ink-700 dark:bg-slate-800 dark:text-slate-300">
              <tr>
                <th className="px-4 py-3 font-medium">{t('news.fields.title')}</th>
                <th className="px-4 py-3 font-medium">{t('news.fields.topic')}</th>
                <th className="px-4 py-3 font-medium">{t('news.fields.claim')}</th>
                <th className="px-4 py-3 font-medium">{t('news.fields.status')}</th>
                ...
                <tr key={item.id} className="dark:hover:bg-slate-700/30">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      <Link to={`/news/${item.id}/edit`} className="btn-secondary text-xs">
                        {t('common.edit')}
                      </Link>
                      {canDelete && (
                        <button
      ...
      <ConfirmDialog
        ...
        message={pendingDelete ? t('news.deleteConfirm', { title: pendingDelete.title }) : ''}
