              ...
              >
                <span className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs text-ink-700/60 dark:text-slate-400">
                    {poll.user_vote_option_id ? `${pct}%` : ''}
                  </span>
                </span>
                {poll.user_vote_option_id && (
...
export function EngagementPage() {
  ...
  const { data: polls = [], isLoading: pollsLoading, error: pollsError } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await engagementService.polls();
      return data;
    },
  });
  ...
  const vote = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.engagementPolls });
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {(vote.error || sign.error) && (
        <div className="mb-6">
          <Alert>{extractError(vote.error || sign.error)}</Alert>
        </div>
      )}
