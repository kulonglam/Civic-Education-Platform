...
test.describe('Live quiz list', () => {
  test('authenticated user sees quizzes from real API', async ({ page, request }) => {
    ...
    await expect(page.getByRole('heading', { name: /quizzes/i })).toBeVisible({ timeout: 15_000 });
    await expect(page.getByText(/South Sudan Civic Basics|Civic Basics/i)).toBeVisible({ timeout: 15_000 });
  });
});
